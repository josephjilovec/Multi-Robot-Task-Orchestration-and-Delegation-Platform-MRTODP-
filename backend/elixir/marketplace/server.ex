...
defmodule MRTODP.Marketplace.Server do
  use Phoenix.Controller, namespace: MRTODP
  use GenServer
  alias Plug.Conn
  require Logger
    ...
    try do
      ...
      case GenServer.call(__MODULE__, {:upload_skill, skill}, @skill_timeout) do
        {:ok, result} ->
          conn
          |> put_status(:created)
          |> json(%{status: "success", message: "Skill uploaded", skill_id: id, result: result})
        {:error, reason} ->
    ...
    try do
      case GenServer.call(__MODULE__, {:download_skill, id}, @skill_timeout) do
        {:ok, skill} ->
          conn
          |> put_status(:ok)
          |> json(%{status: "success", skill: skill})
        {:error, reason} ->
  ...
  defp log_error(message) do
    File.write_append(@log_file, "[#{DateTime.utc_now()}] ERROR: #{message}\n")
  rescue
