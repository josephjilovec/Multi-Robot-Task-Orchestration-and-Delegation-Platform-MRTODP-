...
@click.option('--model-path', default='backend/python/ai_engine/models/task_model.h5',
...
@click.option('--grpc-endpoint', default='localhost:50051',
...
def cli(ctx: click.Context, model_path: str, db_path: str, grpc_endpoint: str, ros_topic: str) -> None:
    ...
    """
    try:
        ctx.obj = TaskDelegator(model_path, db_path, grpc_endpoint, ros_topic)
...
def assign(ctx: click.Context, task_type: str, robot_id: str) -> None:
    ...
    try:
        ...
        delegator: TaskDelegator = ctx.obj
        # Validate robot capabilities (simplified; assumes delegator checks compatibility)
        result = delegator.delegate_task(task_type)  # Note: Manual assignment logic can be extended
        if result['status'] == 'error':
            raise RuntimeError(result['message'])
...
def monitor(ctx: click.Context, task_id: int) -> None:
    ...
    try:
        ...
        delegator: TaskDelegator = ctx.obj
        # Query task status from SQLite (assumes delegator has access to database)
