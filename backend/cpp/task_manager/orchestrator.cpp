...
private:
    ...
    std::string queryAIEngine(const std::string& task_type) {
        ...
        zmq_socket_.send(zmq::buffer(request.dump()), zmq::send_flags::none);

        zmq::message_t reply;
        zmq_socket_.recv(reply);
        auto response = nlohmann::json::parse(std::string(static_cast<char*>(reply.data()), reply.size()));
...
public:
    ...
    Orchestrator() : zmq_context_(1), zmq_socket_(zmq_context_, ZMQ_REQ), ros_node_() {
        ...
        try {
            ...
            throw std::runtime_error("Failed to connect to AI engine: " + std::string(e.what()));
        }
