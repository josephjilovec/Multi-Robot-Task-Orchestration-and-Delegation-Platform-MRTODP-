...
function TaskManager() {
  ...
  const onDragEnd = async (result) => {
    ...
    try {
      await axios.put(`${apiUrl}/api/tasks/reorder`, { tasks: reorderedTasks });
      setError('');
    } catch (err) {
      setError(`Failed to reorder tasks: ${err.response?.data?.message || err.message}`);
      // Revert on failure
      setTasks(tasks);
    }
  };
                  ...
                  <Draggable key={task.id} draggableId={task.id} index={index}>
                    {(provided) => (
                      <div
                        ...
                        className="border p-4 mb-2 rounded-md bg-gray-50 hover:bg-gray-100 transition duration-200"
                      >
                        <p className="text-gray-800">
                          <strong>ID:</strong> {task.id} | <strong>Robot:</strong> {task.robotId} |{' '}
                          <strong>Command:</strong> {task.command} |{' '}
                          <strong>Parameters:</strong> {task.parameters.join(', ')}
                        </p>
                      </div>
                    )}
                  </Draggable>
